...
class ExecutionEngine:
    def __init__(self):
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # PRICE MOCK — price_feed wrapper (DEMO TEST სისტემა)
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # Shell ბრძანებები:
        #   გამორთვა:  echo '{}' > /var/data/price_mock.json
        #   -10%: echo '{"BTC/USDT":69300,"ETH/USDT":2088,"BNB/USDT":565}' > /var/data/price_mock.json
        #   -20%: echo '{"BTC/USDT":61600,"ETH/USDT":1856,"BNB/USDT":502}' > /var/data/price_mock.json
        #   -30%: echo '{"BTC/USDT":53900,"ETH/USDT":1624,"BNB/USDT":440}' > /var/data/price_mock.json
        ...
        if self.mode == "DEMO":
            ...
            try:
                ...
                _mf = _pl.Path(_MOCK_PATH)
                _mf.parent.mkdir(parents=True, exist_ok=True)
                if not _mf.exists():
                    _mf.write_text("{}")
                    logger.info("[PRICE_MOCK] /var/data/price_mock.json created (engine)")
            except Exception as _me:
                logger.warning(f"[PRICE_MOCK] engine_init_fail | err={_me}")

            def _mock_fetch_ticker(symbol, params={}):
                try:
                    ...
                    if _data:
                        ...
                        if _p is not None:
                            _price = float(_p)
                            logger.debug(f"[PRICE_MOCK] engine {symbol} mock={_price:.4f}")
                            return {
                                "last":  _price, "close": _price,
                                "bid":   round(_price * 0.9999, 8),
                                "ask":   round(_price * 1.0001, 8),
                                "high":  round(_price * 1.005,  8),
                                "low":   round(_price * 0.995,  8),
                                "open":  round(_price * 1.002,  8),
                                "symbol": symbol,
                                "info": {"highPrice": str(round(_price * 1.005, 8))},
                            }
        ...
        self.estimated_slippage_pct = 0.0
        self.min_net_profit_pct = _cfg.MIN_NET_PROFIT_PCT

        self.entry_mode = "MARKET"
        self.limit_entry_offset_pct = float(os.getenv("LIMIT_ENTRY_OFFSET_PCT", "0.03"))
        self.limit_entry_timeout_sec = int(os.getenv("LIMIT_ENTRY_TIMEOUT_SEC", "15"))
        ...
        self.max_risk_per_trade_pct = 0.0
        self.capital_usage_min = _cfg.CAPITAL_USAGE_MIN
        self.capital_usage_max = _cfg.CAPITAL_USAGE_MAX
        self.max_portfolio_exposure = 0.0
        self.max_symbol_exposure = 0.0
        ...
        try:
            class _SafeAdapter:
                ...
                def get_trade_logs(self, _signal_id):
                    return ["ENTRY", "OCO", f"EXIT_{str(outcome).upper()}", "PNL"]
                ...
                def get_balance(self):
                    if self.engine.exchange is None:
                        return {}
                    try:
                        return {"USDT": float(self.engine.exchange.fetch_balance_free("USDT"))}
                    except Exception:
                        return {}
    ...
    def reconcile_oco(self) -> None:
        ...
        for r in rows:
            (
                ...
                tp_order_id, sl_order_id, tp_price,
                sl_stop_price, sl_limit_price, amount,
            ...
            try:
                ...
                if tp_status in filled:
                    ...
                    if current_status not in {"CLOSED_TP", "CLOSED_SL"}:
                        close_trade(signal_id, exit_price=exitp, outcome="TP", pnl_quote=pnl_quote, pnl_pct=pnl_pct)
                        set_oco_status(link_id, "CLOSED_TP")

                    log_event("TRADE_CLOSED", f"{signal_id} {symbol} TP exit={exitp:.6f} pnl={pnl_quote:.4f}")
                    logger.info(f"TRADE_CLOSED | id={signal_id} outcome=TP exit={exitp:.6f} pnl={pnl_quote:.4f}")
                    ...
                    try:
                        ...
                        if _dust_sell > 0:
                            ...
                            logger.info(f"DUST_IGNORED_TP | sym={symbol} free_{_dust_base}={_dust_free:.8f} below min")
                    except Exception as _ds_e:
                        logger.debug(f"DUST_SWEEP_TP_FAIL | sym={symbol} err={_ds_e}")

                    try:
                        stats = get_trade_stats()
                        notify_trade_closed(symbol=symbol, entry_price=entry_price, exit_price=exitp, pnl_quote=pnl_quote, pnl_pct=pnl_pct, outcome="TP", stats=stats)
                        try:
                        ...
                        self._run_post_close_diagnostics(
                            signal_id=str(signal_id), link_id=link_id, symbol=str(symbol),
                            qty=float(qty), quote_in=float(quote_in), entry_price=float(entry_price),
                            exit_price=float(exitp), outcome="TP", pnl_quote=float(pnl_quote), pnl_pct=float(pnl_pct),
                            tp_order_id=str(tp_order_id or ""), sl_order_id=str(sl_order_id or ""),
                            tp_price=float(tp_price) if tp_price is not None else None,
                ...
                if sl_status in filled:
                    ...
                    if current_status not in {"CLOSED_TP", "CLOSED_SL"}:
                        close_trade(signal_id, exit_price=exitp, outcome="SL", pnl_quote=pnl_quote, pnl_pct=pnl_pct)
                        set_oco_status(link_id, "CLOSED_SL")

                    log_event("TRADE_CLOSED", f"{signal_id} {symbol} SL exit={exitp:.6f} pnl={pnl_quote:.4f}")
                    logger.info(f"TRADE_CLOSED | id={signal_id} outcome=SL exit={exitp:.6f} pnl={pnl_quote:.4f}")
                    ...
                    try:
                        stats = get_trade_stats()
                        notify_trade_closed(symbol=symbol, entry_price=entry_price, exit_price=exitp, pnl_quote=pnl_quote, pnl_pct=pnl_pct, outcome="SL", stats=stats)
                        try:
                        ...
                        try:
                            ...
                            if _re_instance is not None:
                                ...
                                logger.warning(f"[REGIME_OUTCOME] SL tracked | sym={symbol} consecutive_sl={cons_sl}")
                        except Exception as _re_err:
                        ...
                        try:
                            ...
                            sl_limit = int(os.getenv("SL_COOLDOWN_AFTER_N", "3"))
                            cur_sl   = sl_state["consecutive_sl"]

                            open_links = list_active_oco_links(limit=20)
                            other_links = [
                                lnk for lnk in open_links
                                if str(lnk[1]) != str(signal_id) and str(lnk[2]) != str(symbol)
                            ]

                            if other_links:
                                logger.warning(f"[SL_COOLDOWN] SL hit #{cur_sl} | cancelling {len(other_links)} other open OCO(s)")

                            for olink in other_links:
                                try:
                                    (ol_id, ol_sig, ol_sym, ol_base, ol_tp_oid, ol_sl_oid, *_rest) = olink
                                    if ol_tp_oid:
                                        try:
                                    ...
                                    try:
                                        ...
                                        if free_base > 0:
                                            ...
                                            sell_amt = _safe_sell_amount(self.exchange, str(ol_sym), free_base * self.sell_buffer, _cl_price)
                                            if sell_amt > 0:
                                                self.exchange.place_market_sell(str(ol_sym), sell_amt)
                                                logger.warning(f"[SL_COOLDOWN] market sold | sym={ol_sym} amount={sell_amt}")
                            ...
                            if cur_sl >= sl_limit:
                                logger.warning(f"[SL_COOLDOWN] {cur_sl} consecutive SL — PAUSE active.")
                                log_event("SL_COOLDOWN_PAUSE_ACTIVE", f"consecutive_sl={cur_sl} all_ocos_cancelled=True")
                        ...
                        self._run_post_close_diagnostics(
                            signal_id=str(signal_id), link_id=link_id, symbol=str(symbol),
                            qty=float(qty), quote_in=float(quote_in), entry_price=float(entry_price),
                            exit_price=float(exitp), outcome="SL", pnl_quote=float(pnl_quote), pnl_pct=float(pnl_pct),
                            tp_order_id=str(tp_order_id or ""), sl_order_id=str(sl_order_id or ""),
                            tp_price=float(tp_price) if tp_price is not None else None,
                            sl_stop_price=float(sl_stop_price) if sl_stop_price is not None else None,
                            sl_limit_price=float(sl_limit_price) if sl_limit_price is not None else None,
    ...
    def _execute_sell(self, signal_id: str, symbol: str, signal_hash: str = None) -> None:
        ...
        if sell_amount <= 0:
            ...
            try:
                ...
                if tr:
                    ...
                    close_trade(trade_signal_id, exit_price=dust_exit_price, outcome="DUST_SELL", pnl_quote=round(dust_pnl_quote, 6), pnl_pct=round(dust_pnl_pct, 4))
                    log_event("TRADE_CLOSED_DUST", f"{trade_signal_id} {symbol} DUST_SELL free={free_base:.8f} last={dust_exit_price:.4f} pnl={dust_pnl_quote:.6f}")
                    logger.warning(f"DUST_SELL_AUTO_CLOSE | id={trade_signal_id} symbol={symbol} free_{base_asset}={free_base:.8f} below_min → DB closed as DUST_SELL pnl={dust_pnl_quote:.6f} USDT")
                    try:
                        _notify_sl_tp_outcome("SL", symbol=str(symbol))
                    except Exception:
                        pass
                    try:
                        ...
                        stats = get_trade_stats()
                        notify_trade_closed(symbol=str(symbol), entry_price=float(entry_price), exit_price=dust_exit_price, pnl_quote=dust_pnl_quote, pnl_pct=dust_pnl_pct, outcome="DUST_SELL", stats=stats)
        ...
        try:
            ...
            if tr:
                trade_signal_id, _, qty, quote_in, entry_price, *_ = tr
                pnl_quote, pnl_pct = self._calc_net_pnl(float(quote_in), float(entry_price), float(avg), float(qty))
                close_trade(trade_signal_id, exit_price=float(avg), outcome="MANUAL_SELL", pnl_quote=float(pnl_quote), pnl_pct=float(pnl_pct))
                log_event("TRADE_CLOSED", f"{trade_signal_id} {symbol} MANUAL_SELL exit={avg} net_pnl_quote={pnl_quote:.4f} net_pnl_pct={pnl_pct:.3f}")
                logger.info(f"TRADE_CLOSED | id={trade_signal_id} symbol={symbol} outcome=MANUAL_SELL exit={avg} net_pnl_quote={pnl_quote:.4f} net_pnl_pct={pnl_pct:.3f}")

                try:
                    stats = get_trade_stats()
                    notify_trade_closed(symbol=str(symbol), entry_price=float(entry_price), exit_price=float(avg), pnl_quote=float(pnl_quote), pnl_pct=float(pnl_pct), outcome="MANUAL_SELL", stats=stats)
                    self._run_post_close_diagnostics(
                        signal_id=str(trade_signal_id), link_id=None, symbol=str(symbol),
                        qty=float(qty), quote_in=float(quote_in), entry_price=float(entry_price),
                        exit_price=float(avg), outcome="MANUAL_SELL", pnl_quote=float(pnl_quote), pnl_pct=float(pnl_pct),
                        tp_order_id="", sl_order_id="", tp_price=None, sl_stop_price=None, sl_limit_price=None,
                    )
                except Exception as e:
    ...
    def _place_partial_tp_order(self, signal_id: str, symbol: str, sell_amount: float, buy_avg: float, adaptive: dict, _override_amount: bool = False) -> Optional[str]:
        ...
        try:
            ...
            if tp1_amount <= 0:
                return None
            order = self.exchange.place_limit_sell_amount(symbol=symbol, base_amount=tp1_amount, price=tp1_price)
    ...
    def _check_breakeven(self, signal_id: str, link_id: int, symbol: str, entry_price: float, sl_order_id: str, sl_stop_price: float, sl_limit_price: float, amount: float, adaptive_meta: Optional[dict] = None) -> bool:
        ...
        try:
            ...
            return False
        try:
            new_sl = self.exchange.place_stop_loss_limit_sell(symbol=symbol, base_amount=float(amount), stop_price=float(new_sl_stop), limit_price=float(new_sl_limit))
            new_sl_id = str(new_sl.get("id") or "")
            from execution.db.repository import _execute as _db_exec
            _db_exec("UPDATE oco_links SET sl_order_id=?, sl_stop_price=?, sl_limit_price=?, updated_at=datetime('now') WHERE id=?", (new_sl_id, float(new_sl_stop), float(new_sl_limit), int(link_id)))
    ...
    def _check_trailing_stop(self, signal_id: str, link_id: int, symbol: str, entry_price: float, sl_order_id: str, amount: float, adaptive_meta: Optional[dict] = None) -> bool:
    ...
    def _place_entry_buy(self, symbol: str, quote_amount: float) -> Tuple[Dict[str, Any], float]:
        ...
        if sp is not None and sp > self.max_spread_pct:
            logger.warning(f"[SPREAD_BLOCKED] symbol={symbol} spread={sp:.4f}% > max={self.max_spread_pct:.4f}%")
    ...
    def execute_signal(self, signal: Dict[str, Any]) -> None:
        ...
        if not sync_ok or db_status not in ("ACTIVE", "RUNNING"):
            logger.warning(f"EXEC_BLOCKED | system not ACTIVE/synced | id={signal_id} status={db_status!r} sync_ok={sync_ok}")
        ...
        if self.max_daily_loss_pct > 0:
            try:
                ...
                daily_pnl = sum(float(t.get("pnl_quote", 0) or 0) for t in closed if str(t.get("closed_at", "") or "")[:10] == today)
                daily_quote_in = sum(float(t.get("quote_in", 0) or 0) for t in closed if str(t.get("closed_at", "") or "")[:10] == today)
                if daily_quote_in > 0:
                    daily_loss_pct = abs(min(0, daily_pnl)) / daily_quote_in * 100.0
                    if daily_loss_pct >= self.max_daily_loss_pct:
                        msg = f"EXEC_REJECT | MAX_DAILY_LOSS | daily_loss={daily_loss_pct:.2f}% >= limit={self.max_daily_loss_pct}% | id={signal_id}"
                        logger.warning(msg)
        ...
        if self.ai_signal_threshold > 0 and adaptive:
            ...
            if sig_ai < self.ai_signal_threshold:
                msg = f"EXEC_REJECT | AI_SIGNAL_THRESHOLD | ai={sig_ai:.3f} < threshold={self.ai_signal_threshold} | id={signal_id}"
                logger.warning(msg)
        ...
        if _max_open > 0:
            try:
                if _dca_mode:
                    ...
                    _all_dca = get_all_open_dca_positions() or []
                    _total_open_now = sum(
                        1 for _p in _all_dca
                        if not _re_suffix.search(r'_L\d+$', str(_p.get("symbol", "")))
                    )
                    _source = "dca_positions(L1)"
                else:
                    _total_open_now = len(get_all_open_trades() or [])
                    _source = "trades"

                logger.info(f"[MAX_OPEN_CHECK] source={_source} open={_total_open_now} limit={_max_open} | id={signal_id}")

                if _total_open_now >= _max_open:
                    msg = f"EXEC_REJECT | MAX_OPEN_TRADES | total_open={_total_open_now} >= limit={_max_open} source={_source} | id={signal_id}"
                    logger.warning(msg)
                    log_event("EXEC_REJECT_MAX_OPEN_TRADES", msg)
                    mark_signal_id_executed(signal_id, signal_hash=signal_hash, action="REJECT_MAX_OPEN_TRADES", symbol=str(symbol))
                    return
                else:
                    logger.info(f"[MAX_OPEN_CHECK] OK | open={_total_open_now} < max={_max_open} → proceeding with BUY")
            except Exception as _e:
            ...
            try:
                ...
                if total_balance > 0:
                    ...
                    if exposure_ratio >= self.max_portfolio_exposure:
                        msg = f"EXEC_REJECT | MAX_PORTFOLIO_EXPOSURE | exposure={exposure_ratio:.2%} >= limit={self.max_portfolio_exposure:.2%} | id={signal_id}"
                        logger.warning(msg)
        ...
        try:
            ...
            env_quote = float(os.getenv("BOT_QUOTE_PER_TRADE", "10"))
                ...
                try:
                    ...
                    if _closed >= 10:
                        ...
                        if _wr_mult != 1.0:
                            ...
                            logger.info(f"[ADAPTIVE_SIZE] wr={_wr:.1%} ({_wins}/{_closed}) mult={_wr_mult:.2f} quote: {_prev:.2f} → {quote_amount:.2f}")
                except Exception as _e:
                ...
                try:
                    ...
                    if _closed >= 15:
                        ...
                        if _kelly_full > 0:
                            ...
                            _kelly_quote = min(_kelly_quote, float(os.getenv("MAX_QUOTE_PER_TRADE", "10")))
                            if _kelly_quote < quote_amount:
                                _prev = quote_amount
                                quote_amount = _kelly_quote
                                logger.info(f"[KELLY] p={_p:.2%} R={_R:.2f} f*={_kelly_full:.3f} half={_kelly_half:.3f} frac={_kelly_frac:.2%} quote: {_prev:.2f} → {quote_amount:.2f}")
                        else:
                            logger.info(f"[KELLY] NEGATIVE_EDGE | p={_p:.2%} R={_R:.2f} f*={_kelly_full:.3f} → min_notional")
            ...
            if self.max_risk_per_trade_pct > 0 and balance > 0:
                ...
                if quote_amount > max_by_risk:
                    logger.info(f"[RISK_CAP] quote {quote_amount:.2f} → {max_by_risk:.2f}")
                    quote_amount = max_by_risk

            regime_name = adaptive.get("REGIME", "STATIC") if adaptive else "STATIC"
            logger.info(f"[SIZE_FIX] final_quote={quote_amount} regime={regime_name} regime_quote={regime_quote:.2f} min_notional={min_notional} tp={tp_pct:.3f}% sl={sl_pct:.3f}%")
            ...
            try:
                ...
                if has_active_oco_for_symbol(str(symbol)):
                    if not allow_scaling:
                        ...
                        mark_signal_id_executed(signal_id, signal_hash=signal_hash, action="REJECT_ACTIVE_OCO", symbol=str(symbol))
                        return
                    ...
                    if active_positions >= max_positions:
                        ...
                        mark_signal_id_executed(signal_id, signal_hash=signal_hash, action="REJECT_MAX_POSITIONS", symbol=str(symbol))
                        return
            except Exception as e:
            ...
            logger.info(f"EXEC_LIVE_BUY_OK | id={signal_id} symbol={symbol} quote={quote_amount} avg={buy_avg} regime={regime_name} tp={tp_pct:.3f}% sl={sl_pct:.3f}% order_id={buy.get('id')}")
