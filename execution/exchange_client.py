...
class BinanceSpotClient:
    ...
    def __init__(self):
        ...
        self.max_quote_per_trade = float(os.getenv("MAX_QUOTE_PER_TRADE", "15"))  # BUG#2 FIX: 10→15
    ...
    def place_limit_buy(self, symbol: str, base_amount: float, price: float) -> Dict[str, Any]:
        """
        Limit buy order — PHANTOM OS და LP LIVE mode-ისთვის.
        amount_to_precision + price_to_precision → Binance LOT_SIZE და PRICE_FILTER compliance.
        timeInForce=GTC — order Binance-ზე რჩება fill-ამდე ან cancel-ამდე.
        """
        self._guard(symbol)
        try:
            amt = float(self.exchange.amount_to_precision(symbol, base_amount))
            ...
            return self._with_retry(
                self.exchange.create_order,
                symbol, "limit", "buy", float(amt), float(px),
                {"timeInForce": "GTC"},
                label="LIMIT_BUY"
            )
        except ExchangeClientError:
