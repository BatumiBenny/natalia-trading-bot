...
#   FIX-S7: _fetch_price() — (_L\d+|_LP\d*)$ suffix strip — covers _LP1.._LP5
...
#   FIX-S9: _fetch_price() + check_independent_short_open() + check_mirror_engine_open()
#            PHANTOM OS suffix fix: _LP → _LP\d* (covers _LP1.._LP5)
#            BTC/USDT_LP1 → BTC/USDT (Binance API-ს ეს სიმბოლო არ იცნობს)
#            პრობლემა: regex r'_L\d+\$' → "BTC/USDT_LP" ვერ strip-ავს (_LP != _L\d+)
#            შედეგი: SHORT DCA და MIRROR ENGINE LP position-ებზე იხსნებოდა
...
class FuturesEngine:
    ...
    def _fetch_price(self, symbol: str) -> float:
        ...
        FIX-S7: (_L\\d+|_LP\d*)$ suffix strip.
        ...
        clean_symbol = _re_sym.sub(r'(_L\d+|_LP\d*)$', '', symbol)
    ...
    def check_mirror_engine_open(self) -> None:
        ...
        for pos in long_positions:
            ...
            exchange_sym = _re_mir.sub(r'(_L\d+|_LP\d*)$', '', symbol)
            ...
            if _re_mir.search(r'(_L\d+|_LP\d*)$', symbol):
                continue
