...
def _read_outbox(path: str) -> Dict[str, Any]:
    ...
    # ── ფაილი ცარიელია (0 bytes) ───────────────────────────────
    # მიზეზი: Render restart, atomic write შეწყვეტა, ან
    # signal_generator-ი ჯერ არ დაწერია.
    # json.load("") → JSONDecodeError → OUTBOX_POP_FAIL loop spam.
    # გამოსწორება: ცარიელი ფაილი = ცარიელი outbox, არა error.
    if os.path.getsize(path) == 0:
        logger.debug("[OUTBOX] empty file → treating as empty outbox")
        return {"signals": []}

    # ── ფაილი corrupt / invalid JSON ───────────────────────────
    # მიზეზი: ნახევრად დაწერილი ფაილი (disk full, SIGKILL mid-write).
    # atomic write (_atomic_write_json) ამ პრობლემას ამომავლობაში
    # გამოირიცხავს, მაგრამ ძველი ფაილები ან external writes corrupt
    # შეიძლება იყოს — safe fallback აუცილებელია.
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
    except (json.JSONDecodeError, ValueError) as e:
        logger.warning(
            f"[OUTBOX] corrupt JSON → resetting | path={path} err={e}"
        )
        # corrupt ფაილი → გადავწეროთ ცარიელ outbox-ად
        try:
            _atomic_write_json(path, {"signals": []})
        except Exception as _we:
            logger.warning(f"[OUTBOX] reset_write_fail | err={_we}")
        return {"signals": []}
