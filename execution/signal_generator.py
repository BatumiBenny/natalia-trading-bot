...
CANDLE_LIMIT = int(os.getenv("BOT_CANDLE_LIMIT", "100"))        # ENV=100
...
MAX_QUOTE_PER_TRADE = float(os.getenv("MAX_QUOTE_PER_TRADE", "15"))   # ENV=15
...
MAX_TRADES_PER_DAY  = int(os.getenv("MAX_TRADES_PER_DAY",  "10000"))  # ENV=10000
MAX_TRADES_PER_HOUR = int(os.getenv("MAX_TRADES_PER_HOUR", "500"))   # ENV=500
...
DYNAMIC_SIZE_MIN      = float(os.getenv("DYNAMIC_SIZE_MIN", "15.0"))
...
def _trades_today_count() -> int:
    """დღეს (UTC) დახურული trade-ების რაოდენობა DB-დან.
    BUG#6 FIX: open_trades ამოღებულია daily count-იდან.
    ადრე: closed_today + ALL open_trades → 18 PHANTOM open → count=18 → MAX_TRADES_PER_DAY=10 hit.
    ახლა: მხოლოდ დღეს დახურული trades ითვლება — PHANTOM open positions არ ბლოკავს.
    """
    try:
        ...
        # მხოლოდ დღეს დახურული trade-ები
        ...
        try:
            ...
            if _dca_mode:
                ...
                _total_open = sum(
                    ...
                    if not _re_sg_cnt.search(r'(_L\d+|_LP\d*)$', str(_p.get("symbol", "")))
                )
            else:
